...
页面默认汇总本月全部任务，也可切换本周；明细最多列出 20 条但总量不截断。大字展示周期累计预计节省的 Token 与 API 等价金额，旁边小字显示所选会话首轮估算、周期 turn 数和响应数。费用默认按价格表中最高价模型估算，另有低调的“按实际使用模型”切换。累计按每条响应的目标文本与缓存边界计算；压缩、缺失上下文或价格会明确标注覆盖缺口，不简单按 turn 数相乘。
