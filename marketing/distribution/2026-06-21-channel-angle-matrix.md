...
Current baseline, checked 2026-06-30 19:57 Asia/Shanghai:
...
- Discovery topics: ai-agent, ai-tools, claude-code, cli, codex, coding-agents, cursor, developer-tools, github-copilot, llmops, prompt-engineering, security, skills
...
```text
I shipped skill-doctor v0.3.4, a local CLI for auditing AI agent skills/rules/instructions before they quietly become a messy config layer.

It scans local files for duplicates, overlapping triggers, risky wording such as credential exposure or upload instructions, cross-agent drift, and always-on context cost.
...
- risky instructions
- estimated context token tax from always-on instruction files
...
```text
skill-doctor v0.3.4 has a safe demo project for AI agent skill/rule conflicts and context-cost checks.
...
It shows discovered instruction files, a trigger conflict, audit findings, and estimated always-on context cost without touching your real agent setup.
```
...
- Dedicated context-cost post draft: `marketing/distribution/2026-06-30-v0.3.4-context-cost-post.md`
