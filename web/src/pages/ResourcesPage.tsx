...
export function ResourcesPage({ snapshot, openResource }: { snapshot: DoctorSnapshot | null; openResource: (resource: UiResource) => void }) {
  ...
  return <section><PageHeading title={t('resources.title')} subtitle={t('resources.subtitle')}><span className="result-count">{t('resources.count', { count: resources.length })}</span></PageHeading>
    <FilterBar query={query} setQuery={setQuery} placeholder={t('resources.search')}>
      <select aria-label={t('resources.type')} value={kind} onChange={(event) => setKind(event.target.value)}><option value="all">{t('resources.allTypes')}</option>{['skill', 'instruction', 'rule', 'prompt', 'agents', 'mcp', 'plugin', 'memory'].map((value) => <option key={value} value={value}>{resourceKindLabel(value)}</option>)}</select>
      <select aria-label={t('resources.platform')} value={platform} onChange={(event) => setPlatform(event.target.value)}><option value="all">{t('resources.allPlatforms')}</option>{platforms.map((value) => <option key={value}>{value}</option>)}</select>
      <select aria-label={t('resources.scope')} value={scope} onChange={(event) => setScope(event.target.value)}><option value="all">{t('resources.allScopes')}</option><option value="project">{t('label.project')}</option><option value="global">{t('label.global')}</option></select>
      <select aria-label={t('resources.status')} value={status} onChange={(event) => setStatus(event.target.value)}><option value="active">{t('resources.activeOnly')}</option><option value="all">{t('resources.allStatuses')}</option><option value="attention">{t('resource.attention', { count: 0 })}</option><option value="disabled">{t('resource.disabled')}</option><option value="unknown">{t('resource.unknown')}</option></select>
    </FilterBar>
