...
export function ContextPage({ snapshot, openResource, onToggle, active = true, excludeCodexSkills = false }: { snapshot: DoctorSnapshot | null; openResource: (resource: UiResource) => void; onToggle: (item: ContextCostItem) => Promise<void>; active?: boolean; excludeCodexSkills?: boolean }) {
  const { t } = useTranslation();
  const [showDisabled, setShowDisabled] = useState(false);
  const context = snapshot?.context;
  const isCodexSkill = (item: ContextCostItem) => item.platform === 'codex' && (item.resource === 'skill' || item.source === 'skill' || item.controlMethod === 'skills.config' || item.kind === 'codex-skill-list' || item.kind === 'plugin-skill-list');
  const items = (showDisabled ? [...(context?.items ?? []), ...(context?.disabledItems ?? [])] : context?.items ?? []).filter((item) => !excludeCodexSkills || !isCodexSkill(item));
  ...
  return <section className="context-page">{!excludeCodexSkills && <><PageHeading title={t('context.title')} subtitle={t('context.subtitle')}><StatusPill kind={!context ? 'warning' : context.summary.overBudget ? 'danger' : contextHasUnknown ? 'warning' : 'success'}>{!context ? t('context.unavailable') : context.summary.overBudget ? t('context.overBudget') : contextHasUnknown ? t('context.partial') : t('context.withinBudget')}</StatusPill></PageHeading>
    ...
    <div className="cost-list">{items.map((item) => {
      ...
      const canToggle = item.controllable === true && Boolean(item.id) && !item.sourcePaths?.length && !isCodexSkill(item);
