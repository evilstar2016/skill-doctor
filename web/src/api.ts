...
export const applyOptimizationChange = (projectDir: string, sessionId: string, preview: OptimizationPreview): Promise<OptimizationOperation> => request('/api/optimization', { method: 'POST', body: JSON.stringify({ action: 'apply', projectDir, sessionId, targets: preview.targets, enabled: preview.after, confirmation: preview.confirmation }) });
