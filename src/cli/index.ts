...
export async function main(argv: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (command === 'cost' || command === 'context') {
    ...
    if (projectDir === 'invalid') {
      process.stderr.write('Usage: skill-doctor cost [project-dir] [--platform <agent>] [--scope project|global|all] [--source skill|mcp|all] [--budget-tokens N] [--fail-on-budget] [--json]\n');
      process.exitCode = 1;
      return;
    }
    ...
    const skills = source === 'mcp'
      ? []
      : filterEntriesByPlatform(filterEntriesByScope(await scanSkills(projectDir, { extraPaths }), scope), platform);
...
function getHelpText(): string {
  return [
    ...
    '  skill-doctor cost [project-dir] [--platform PLATFORM] [--scope project|global|all] [--source skill|mcp|all] [--budget-tokens N] [--fail-on-budget] [--json]',
    '  skill-doctor context [project-dir] [--platform PLATFORM] [--scope project|global|all] [--source skill|mcp|all] [--budget-tokens N] [--fail-on-budget] [--json]',
...
function readCostProjectDir(args: string[], cwd: string, implicitPlatform: Platform | null = null): string | 'invalid' {
  ...
  const valueFlags = new Set(['--scope', '--budget-tokens', '--platform', '--source']);
...
function readImplicitCostPlatform(args: string[], cwd: string): Platform | null {
  const positionals = readPositionals(args, new Set(['--scope', '--budget-tokens', '--platform', '--source']));
