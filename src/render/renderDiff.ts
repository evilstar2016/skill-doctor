...
export function renderDiff(result: DiffResult): string {
  ...
  // 功能覆盖
  lines.push(section('功能覆盖'));
  if (analysis.coverageOverlap.length > 0) {
    lines.push(`  ${gray('共同覆盖')}   ${analysis.coverageOverlap.join('、')}`);
  }
  if (analysis.coverageOnlyA.length > 0) {
    lines.push(`  ${gray(`仅 ${skillA.name}`)}   ${analysis.coverageOnlyA.join('、')}`);
  }
  if (analysis.coverageOnlyB.length > 0) {
    lines.push(`  ${gray(`仅 ${skillB.name}`)}   ${analysis.coverageOnlyB.join('、')}`);
  }
  lines.push('');

  // 优缺点
  lines.push(section('优缺点'));
  ...
  // 分情况建议
  lines.push(section('分情况建议'));
  for (const advice of analysis.situationalAdvice) {
    const rec = advice.recommendation === 'A' ? bold(skillA.name) : advice.recommendation === 'B' ? bold(skillB.name) : bold('均可');
    lines.push(`  ${advice.condition.padEnd(32)} → 选 ${rec}`);
