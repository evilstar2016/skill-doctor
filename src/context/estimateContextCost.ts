...
function appendCodexSkillListAggregates(
  ...
  for (const [key, groupItems] of groups) {
    ...
    aggregateItems.push({
      ...
      controllable: resource === 'skill' ? false : groupItems.every((item) => item.controllable !== false),
      controlStatus: resource === 'skill' ? 'not-controllable' : groupItems.every((item) => item.controlStatus === 'configured') ? 'configured' : 'unknown',
...
function estimateMcpServerCost(server: McpServerRecord, tokenCounter: TokenCounter): ContextCostItem {
  ...
  return {
    ...
    scope: server.scope,
    source: server.context?.resource === 'plugin' ? 'plugin' : 'mcp',
    resource: server.context?.resource ?? 'mcp',
    ...(server.context?.configSource ? { configSource: server.context.configSource } : {}),
    kind: server.context?.resource === 'plugin' ? 'plugin-mcp-tool-list' : 'mcp-tool-list',
    estimatedTokens,
